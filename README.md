...
- **Every page requires a signed-in user.** There are two roles: **viewer** (read-only)
  ...
  **operator** role — see "Deployment: the first operator account" below for how to
  create the first account. Finer-grained RBAC / SSO is out of scope — see the roadmap in
  `docs/superpowers/specs/2026-09-05-kamal-panel-design.md`.
...
The full design rationale — architecture, data model, security posture, UI structure,
and a running log of decisions and known limitations — lives in
`docs/superpowers/specs/2026-09-05-kamal-panel-design.md`. If something above seems
surprising, that document almost certainly explains why.
